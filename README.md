...
Unofficial Home Assistant integration for **ENGIE Particuliers**. It exposes the current electricity and gas prices so you can feed the Energy dashboard with *Use an entity with the current price*.
...
One Home Assistant **device per energy contract**. On each device:

| Entity | Unit | Energy dashboard |
|--------|------|------------------|
| **Price kWh TTC** | `EUR/kWh` | Use this for *current price* |
| Price kWh HT | `EUR/kWh` | Optional |
| Subscription TTC | `EUR` | Monthly standing charge, not a kWh price |
| `{cadran}` price kWh TTC / HT | `EUR/kWh` | BASE / HP / HC when ENGIE reports bands |

Electricity currently comes from the app contractual rate. Gas is derived from the latest billed month and can move; the standing charge is a separate entity.
...
2. The coordinator refreshes prices on an interval (default 6 hours)
...
Home Assistant will install `ha-engie-api` automatically from `manifest.json`.
